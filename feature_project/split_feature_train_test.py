...
feature_total_data_path = 'data/feature_total_data_0413.csv'
...
train_data.to_csv('data/feature_train_data_0413.csv', index=False)

print('训练集的列数为：{0}，行数为：{1}'.format(train_data.columns.size,
                                   train_data.iloc[:, 0].size))

test_data.to_csv('data/feature_test_data_0413.csv', index=False)
