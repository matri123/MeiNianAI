...
def xgb_cv(X_train, Y_train):
    cv_params = {
        ...
        # 'colsample_bytree': [0.01, 0.05, 0.1, 0.15, 0.2],
...
def model_process(X_train, train_label_shousuo, train_label_shuzhang,
    ...
    test_label.to_csv('submit_result_0413.csv', index=False, header=None)


if __name__ == '__main__':
    print("***************************开始导入数据集*******************************")
    feature_train_data = pd.read_csv('data/feature_train_data_3.csv')
    feature_test_data = pd.read_csv('data/feature_test_data_3.csv')
